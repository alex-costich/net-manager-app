...
def compare_neighbors_with_parent(id_parent_pairs, neighbors_values):
    ...
    for device_id, parent_id in id_parent_pairs.items():
        ...
        if device_id in neighbors_values:
            ...
            if parent_id in neighbors_values:
                parent_neighbors = neighbors_values[parent_id]
                for neighbor_pair in device_neighbors:
                    if neighbor_pair in parent_neighbors or neighbor_pair[::-1] in parent_neighbors:
                        flipped_pairs[device_id].append(neighbor_pair)
...
print(json_data)
print(flipped_pairs)  # Print the flipped pairs to see if device 3 is included
print(id_parent_pairs)  # Print the id_parent_pairs dictionary
